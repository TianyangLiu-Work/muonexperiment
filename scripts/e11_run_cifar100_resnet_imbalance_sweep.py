...
DEFAULT_OUTPUT_DIR = Path("results/e11_cifar100_resnet_imbalance_sweep")
DEFAULT_FIGURE_DIR = Path("figures/e11_cifar100_resnet_imbalance_sweep")
DEFAULT_DISCUSSION_PATH = Path("discussion/e11_cifar100_resnet_imbalance_sweep.md")
DEFAULT_TAIL_TRAIN_PER_CLASS = (10, 30, 100, 300)
...
def run_sweep(
    ...
    *,
    tail_train_per_class: tuple[int, ...],
    progress: bool,
    ...
    for tail_count in tail_train_per_class:
        config = replace(base_config, tail_train_per_class=tail_count)
        imbalance_ratio = float(base_config.head_train_per_class) / float(tail_count)
        if progress:
            print(
                f"[resnet-imbalance-sweep] tail_train_per_class={tail_count} "
                f"imbalance_ratio={imbalance_ratio:.4g}",
                flush=True,
            )
        ...
        for frame in (step_metrics, pair_summary, layer_metrics):
            frame["tail_train_per_class"] = int(tail_count)
            frame["imbalance_ratio"] = imbalance_ratio
...
def write_figure(pair_summary: pd.DataFrame, figure_dir: Path) -> Path:
    ...
    ordered = pair_summary.sort_values("tail_train_per_class")
    x = ordered["tail_train_per_class"].to_numpy(dtype=float)
    ...
    axes[0].set_xlabel("tail train examples per class")
    axes[0].set_ylabel("spectral / Fro squared tail-drift ratio")
    axes[0].set_title("Matched-gain drift across imbalance")
    ...
    axes[1].set_xlabel("tail train examples per class")
    axes[1].set_ylabel("pre-update tail quality")
    axes[1].set_ylim(bottom=0.0)
    axes[1].set_title("Tail quality across imbalance")
    axes[1].legend(frameon=False)

    fig.suptitle("CIFAR-100-LT ResNet18 imbalance sweep")
    fig.tight_layout()
    path = figure_dir / "cifar100_resnet_imbalance_sweep.png"
...
def write_discussion(
    ...
    best_tail = ordered.loc[ordered["mean_tail_accuracy_before"].idxmax()]
    table_columns = [
    ...
    lines = [
        ...
        "",
        f"- Seeds per tail count: {len(base_config.seeds)}",
        f"- Head train examples per class: {base_config.head_train_per_class}",
        f"- Warmup steps per setting: {base_config.warmup_steps}",
        ...
        "",
        f"![CIFAR-100-LT ResNet18 imbalance sweep](../{figure_path.as_posix()})",
        "",
        "## Summary",
        "",
        ...
        f"at tail_train_per_class={int(worst_drift['tail_train_per_class'])}.",
        f"- Best pre-update tail accuracy: {fmt(best_tail['mean_tail_accuracy_before'])} "
        f"at tail_train_per_class={int(best_tail['tail_train_per_class'])}.",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tail-train-per-class", type=parse_int_list, default=DEFAULT_TAIL_TRAIN_PER_CLASS)
...
def main() -> None:
    ...
    tail_counts = (5, 10) if args.smoke else args.tail_train_per_class
    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    step_metrics, pair_summary, layer_metrics = run_sweep(
        config,
        tail_train_per_class=tail_counts,
    ...
    (output_dir / "config.json").write_text(
        json.dumps({"base_config": asdict(config), "tail_train_per_class": list(tail_counts)}, indent=2),
        encoding="utf-8",
    )
    ...
    print(f"saved CIFAR-100-LT ResNet18 imbalance sweep to {output_dir}")
