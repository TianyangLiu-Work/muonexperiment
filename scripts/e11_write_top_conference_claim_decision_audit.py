...
def build_claim_decision_matrix(sources: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    tuned_seal_boundary = sources["tuned_protocol_seal_boundary"]
    tuned_seal_gates = sources["tuned_protocol_seal_gates"]
    tuned_seal_immutability = sources["tuned_protocol_seal_immutability"]
    tuned_fairness_gates = sources["tuned_fairness_gates"]
    tuned_fairness_budget = sources["tuned_fairness_budget"]
    tuned_robustness_gates = sources["tuned_final_robustness_gates"]
    tuned_robustness_tests = sources["tuned_final_robustness_tests"]
    ...
    muon_budget = int(
        tuned_fairness_budget[
            tuned_fairness_budget["budget_interpretation"].eq("candidate_search_budget_disclose_not_final_advantage")
        ]["setting_count"].sum()
    )
    baseline_budget = int(
        tuned_fairness_budget[
            tuned_fairness_budget["budget_interpretation"].eq("tuned_baseline_surface")
        ]["setting_count"].sum()
    )
        ...
        [
            ...
            },
            {
                ...
                "evidence_status": (
                    ...
                    f"{status_line(tuned_seal_boundary, 'boundary_id')}; "
                    f"{status_line(tuned_seal_gates, 'gate_id')}; "
                    f"{status_line(tuned_fairness_gates, 'gate_id')}; "
                    f"validation_budget_baselines={baseline_budget}; validation_budget_muon_candidates={muon_budget}; "
                    f"{status_line(tuned_robustness_gates, 'gate_id')}; "
                    f"robustness_test_rows={len(tuned_robustness_tests)}; "
                ...
                "author_allowed_wording": (
                    ...
                    "permits only sequential progress accounting from the contiguous prefix; the protocol hash seal "
                    "freezes registry, seed split, selection objective, runner, and final-gate surfaces after partial "
                    "validation exposure; the fairness audit discloses Muon candidate-budget asymmetry and requires "
                    "shared seeds, metrics, multiplicity, and reporting surfaces; the final robustness plan fixes "
                    "nonparametric sensitivity checks, dropped-seed policy, and claim downgrades before final rows; "
                    "final execution, evaluator, and launch audits are dry-run/not_ready boundaries until final rows exist"
                ),
                "author_blocked_wording": (
                    ...
                    "hiding validation-budget asymmetry or comparing Muon only to undertuned baselines; "
                    "claiming robust final performance without sign-flip/bootstrap sensitivity and complete paired seeds; "
                ...
                "decisive_gate": (
                    ...
                    "leakage guards, VRF refresh-firewall transitions, TPS protocol-seal gates, and TBF "
                    "fairness gates plus RBP final-robustness gates still passing"
                ),
                "required_next_action": (
                    ...
                    "validation refresh firewall, protocol seal, fairness audit, final robustness plan, and "
        ...
        [
            ...
            },
            {
                ...
                "current_response": (
                    ...
                    "occupancy logging, and final seeds finish. The tuned leakage audit, validation "
                    "refresh firewall, protocol hash seal, fairness audit, final robustness plan, and final execution/evaluator/launch audits "
                    "separately block optional-stopping moves, protocol drift after partial exposure, "
                    "hidden validation-budget asymmetry, fragile parametric-only claims, premature final-safe-submit, "
                    "and final-performance claims from partial validation."
                ),
                ...
                "forbidden_shortcut": (
                    ...
                    "validation refresh firewall forbidden-action matrix or changing sealed protocol surfaces "
                    "after partial exposure without a fresh protocol or hiding validation-budget asymmetry or claiming "
                    "robust final performance without sign-flip/bootstrap sensitivity and complete paired seeds or running "
        ...
        [
            ...
            },
            {
                ...
                "manuscript_edit": (
                    ...
                    "occupancy logging, leakage guards, validation refresh firewall transitions, protocol-seal gates, fairness gates, robustness gates, final execution/evaluation/launch gates, and "
        ...
        [
            ...
            },
            {
                ...
                "edit_action": (
                    ...
                    "state-distribution contract, tuned leakage audit, validation refresh firewall, protocol hash seal, fairness audit, and final robustness plan, and quarantine all competitive "
                ...
                "acceptance_check": (
                    ...
                    "state-distribution occupancy summaries, passing leakage guards, passing refresh-firewall transitions, passing protocol-seal gates, disclosed fairness-budget gates, and passing robustness sensitivity gates for selected recipes "
