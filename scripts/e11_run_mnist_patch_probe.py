...
OUTPUT_DIR = Path("results/e11_mnist_patch_probe")
FIGURE_DIR = Path("figures/e11_mnist_patch_probe")
DISCUSSION_PATH = Path("discussion/e11_mnist_patch_probe.md")
            ...
            for lr in [3e-3, 1e-2]:
                specs.append(
                    ProblemSpec(
                        ...
                        setting=f"MNIST patch filters={hidden_dim} kernel={kernel_size} stride=2 lr={lr:.0e}",
                        steps=5,
                        d=2,
                        rank=kernel_size,
...
def plot_summary(summary: pd.DataFrame) -> Path:
    ...
    path = FIGURE_DIR / "mnist_patch_first_order_ratios.png"
    ...
    for ax, kernel_size in zip(axes, [5, 7]):
        ...
        ax.set_xscale("log")
        ax.set_title(f"{kernel_size}x{kernel_size} patches")
...
def write_discussion(summary: pd.DataFrame, calibration: pd.DataFrame, spectrum: pd.DataFrame, figure: Path) -> None:
    ...
    nr = require_one(spectrum, problem_family="MNISTPatchClassifier", metric="nrUpdate")
    st = require_one(spectrum, problem_family="MNISTPatchClassifier", metric="stUpdate")
    text = f"""# E11 MNIST Patch Classifier Probe
...
![MNIST patch first-order ratios](../{figure})
...
def main() -> None:
    ...
    print(f"saved MNIST patch probe to {OUTPUT_DIR}")
