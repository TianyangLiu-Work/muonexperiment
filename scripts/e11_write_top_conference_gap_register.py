...
def gap_rows() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "current_state": "The synthetic sandwich-block condition gives the right sign boundary, discussion/e11_condition_score_theory_bridge.md separates the supported threshold-direction guardrail from failed residual-ranking targets, discussion/e11_condition_score_failure_mechanism_audit.md shows the current measurable score reverses on held-out ResNet50 bottleneck/downsample parameterization, discussion/e11_condition_score_v4_protocol.md registers a two-axis transport protocol before unspent final splits, discussion/e11_condition_score_v4_validation_freeze.md freezes the amplitude-minus-direction residual score after the validation-only split, discussion/e11_condition_score_v4_final_evaluation.md shows that this score still reverses on the CIFAR-10 mixed final data split, discussion/e11_condition_score_v4_failure_mechanism_audit.md shows the v4 reversal is not a direction-threshold failure but an amplitude/depth aggregation failure, and discussion/e11_condition_score_v5_theory_protocol.md states the sandwiched tail-drift contract plus a required partition/architecture transport term.",
            "required_next_evidence": "Implement a validation-freeze evaluator for the v5 transport-normalized score, then run the registered v5 unspent splits and preserve negative outcomes if the transport term fails.",
