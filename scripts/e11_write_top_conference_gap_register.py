...
def gap_rows() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "current_state": "The natural head-to-tail boundary audit in discussion/e11_natural_head_tail_boundary.md scans committed digits and CIFAR-100-LT ResNet matched-head-gain summaries with a fixed CI rule. It reports no strict natural primary full tail-output drift counterexample in 37 primary rows, while recording component true-logit and secondary loss/margin/accuracy tradeoff candidates as claim-boundary evidence rather than as primary drift failures.",
            "required_next_evidence": "Run a fresh pre-registered natural negative-search protocol over additional architectures, data partitions, checkpoints, and target head-gain fractions using the same CI stopping rule; preserve null outcomes and keep component/outcome tradeoffs separate from the primary full-drift claim.",
            "acceptance_gate": "For any found boundary case, include the same drift, tail-loss, margin, rank, JVP, checkpoint-quality, and pre-update tail-quality table used for positive cases, plus an explicit label for primary full-drift failures versus component or outcome tradeoffs.",
            "compute_mode": "CPU plus optional GPU via Slurm",
            "planned_artifacts": "results/e11_natural_head_tail_boundary/*; discussion/e11_natural_head_tail_boundary.md",
