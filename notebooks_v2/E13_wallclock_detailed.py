...
==================================================
MS only. ALGOS=["Muon-Exact","Muon-RandSVD","Muon-Trunc","SGD"]. D=50. SEEDS=10.
"""
import os as _os
...
ALGOS       = ["Muon-Exact", "Muon-RandSVD", "Muon-Trunc", "SGD"]
DIMS        = [50]
R           = 5
LR          = 0.01
NOISE       = 0.0
...
def run_ms(algo, d, r, lr, noise, dist, spectrum, kappa, init_scale, seed, iters):
    ...
    # ── Detailed logger (v2) ─────────────────────────
    logger = DetailedLogger(LOG_DIR, "E13_detailed", algo, {k: v for k, v in locals().items()
...
if __name__ == "__main__":
    ...
    print(f"E13: {len(ALGOS)} algos x {len(SEEDS)} seeds = {total} runs")
    print()
    rows = []
    idx = 0
    d = DIMS[0]
    for algo in ALGOS:
        for seed in SEEDS:
            idx += 1
            print(f"[E13 {idx}/{total}] {algo} seed={seed}", flush=True)
            row = run_ms(algo, d, R, LR, NOISE, DIST, SPECTRUM, KAPPA,
                         INIT_SCALE, seed, ITERS)
            rows.append(row)

    save_csv("E13_detailed", rows)

    print(f"\n{'='*60}")
    print("E13 统计汇总 — Wall-clock Time")
    print(f"{'='*60}")
    print(f"\n{'algo':<16s} {'n':>4s} {'avg time_s':>10s} {'std time_s':>10s} {'avg K_eps':>10s} {'avg loss':>12s}")
    print("-" * 65)
    for algo in ALGOS:
        subset = [r for r in rows if r["algo"] == algo]
        nn = len(subset)
        avg_t = np.mean([r["time_s"] for r in subset])
        std_t = np.std([r["time_s"] for r in subset])
        avg_k = np.mean([r["K_epsilon"] for r in subset])
        avg_loss = np.mean([r["final_loss"] for r in subset])
        print(f"{algo:<16s} {nn:>4d} {avg_t:>10.3f} {std_t:>10.3f} {avg_k:>10.1f} {avg_loss:>12.2e}")

    print(f"\n{'='*60}")
    print("配对检验 (time_s vs Muon-Exact)")
    print(f"{'='*60}")
    muon_t = [r["time_s"] for r in rows if r["algo"] == "Muon-Exact"]
    for algo in [a for a in ALGOS if a != "Muon-Exact"]:
        other_t = [r["time_s"] for r in rows if r["algo"] == algo]
        if len(muon_t) == len(other_t) and len(muon_t) > 1:
            t_stat, p_val = paired_ttest(muon_t, other_t)
            d_effect = compute_effect_size(muon_t, other_t, paired=True)
    ...
    m = int(2 * d * R)
    for algo in ALGOS:
        algo_map = {"Muon-Exact": "Muon-Exact", "Muon-RandSVD": "Muon-Exact",
                    "Muon-Trunc": "Muon-Exact", "SGD": "SGD"}
