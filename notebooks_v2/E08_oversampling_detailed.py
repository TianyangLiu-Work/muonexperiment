"""
E08 — Oversampling/undersampling: measurement count effect
==================================================
MS only. ALGOS=["Muon-Exact","SGD"]. D=50, R=5. GAMMAS=[0.5,1.0,2.0,3.0,5.0]. SEEDS=8.
"""
import os as _os
...
def run_ms(algo, d, r, lr, noise, dist, spectrum, kappa, init_scale, seed, iters, gamma):

    # ── Detailed logger (v2) ─────────────────────────
    logger = DetailedLogger(LOG_DIR, "E08_detailed", algo, {k: v for k, v in locals().items()
...
if __name__ == "__main__":
    total = len(ALGOS) * len(GAMMAS) * len(SEEDS)
    print(f"E08: {len(ALGOS)} algos x {len(GAMMAS)} gammas x {len(SEEDS)} seeds = {total} runs")
    print()
    rows = []
    idx = 0
    d = DIMS[0]
    for algo in ALGOS:
        for gamma in GAMMAS:
            for seed in SEEDS:
                idx += 1
                print(f"[E08 {idx}/{total}] {algo} gamma={gamma} seed={seed}", flush=True)
                row = run_ms(algo, d, R, LR, NOISE, DIST, SPECTRUM, KAPPA,
                             INIT_SCALE, seed, ITERS, gamma)
                rows.append(row)

    save_csv("E08_detailed", rows)

    print(f"\n{'='*60}")
    print("E08 统计汇总")
    print(f"{'='*60}")
    print(f"\n{'algo':<16s} {'gamma':>6s} {'m':>8s} {'n':>4s} {'avg K_eps':>10s} {'avg loss':>12s}")
    print("-" * 70)
    d = DIMS[0]
    for algo in ALGOS:
        for gamma in GAMMAS:
            subset = [r for r in rows if r["algo"] == algo and abs(r["gamma"] - gamma) < 1e-9]
            n = len(subset)
            if n == 0: continue
            m = int(gamma * d * d)
            avg_k = np.mean([r["K_epsilon"] for r in subset])
            avg_loss = np.mean([r["final_loss"] for r in subset])
            print(f"{algo:<16s} {gamma:>6.1f} {m:>8d} {n:>4d} {avg_k:>10.1f} {avg_loss:>12.2e}")

    print(f"\n{'='*60}")
    print("配对检验: Muon-Exact vs SGD (K_epsilon)")
    print(f"{'='*60}")
    for gamma in GAMMAS:
        muon_k = [r["K_epsilon"] for r in rows if r["algo"] == "Muon-Exact" and abs(r["gamma"] - gamma) < 1e-9]
        sgd_k  = [r["K_epsilon"] for r in rows if r["algo"] == "SGD" and abs(r["gamma"] - gamma) < 1e-9]
        if len(muon_k) == len(sgd_k) and len(muon_k) > 1:
            ...
            m = int(gamma * d * d)
            print(f"  gamma={gamma:.1f} m={m:>5d}: t={t_stat:+.3f} p={p_val:.4f} {sign}  Cohen's d={d_effect:+.2f}")
    print(f"\n{'='*60}")
    print("E08 完成")
    print(f"{'='*60}")
