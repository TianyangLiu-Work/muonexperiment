"""
E04 — Init x Noise sweep: Muon vs SGD
==================================================
MS only. ALGOS=["Muon-Exact","SGD"]. D=50. INIT_SCALES=[0.001,0.01,0.1,1.0]. NOISES=[0,0.01,0.1]. SEEDS=5.
"""
import os as _os
...
R           = 5
LR          = 0.01
INIT_SCALES = [0.001, 0.01, 0.1, 1.0]
NOISES      = [0, 0.01, 0.1]
DIST        = "normal"
...
def run_ms(algo, d, r, lr, noise, dist, spectrum, kappa, init_scale, seed, iters):
    ...
    # ── Detailed logger (v2) ─────────────────────────
    logger = DetailedLogger(LOG_DIR, "E04_detailed", algo, {k: v for k, v in locals().items()
...
if __name__ == "__main__":
    total = len(ALGOS) * len(INIT_SCALES) * len(NOISES) * len(SEEDS)
    print(f"E04: {len(ALGOS)} algos x {len(INIT_SCALES)} init_scales x {len(NOISES)} noises x {len(SEEDS)} seeds = {total} runs")
    print()
    rows = []
    idx = 0
    d = DIMS[0]
    for algo in ALGOS:
        for init_scale in INIT_SCALES:
            for noise in NOISES:
                for seed in SEEDS:
                    idx += 1
                    print(f"[E04 {idx}/{total}] {algo} init={init_scale} noise={noise} seed={seed}", flush=True)
                    row = run_ms(algo, d, R, LR, noise, DIST, SPECTRUM, KAPPA,
                                 init_scale, seed, ITERS)
                    rows.append(row)

    save_csv("E04_detailed", rows)

    print(f"\n{'='*60}")
    print("E04 统计汇总")
    print(f"{'='*60}")
    print(f"\n{'algo':<16s} {'init_scale':>10s} {'noise':>6s} {'n':>4s} {'avg K_eps':>10s} {'avg loss':>12s}")
    print("-" * 60)
    for algo in ALGOS:
        for init_scale in INIT_SCALES:
            for noise in NOISES:
                subset = [r for r in rows if r["algo"] == algo and abs(r["init_scale"] - init_scale) < 1e-9 and abs(r["noise"] - noise) < 1e-9]
                n = len(subset)
                if n == 0: continue
                avg_k = np.mean([r["K_epsilon"] for r in subset])
                avg_loss = np.mean([r["final_loss"] for r in subset])
                print(f"{algo:<16s} {init_scale:>10.4f} {noise:>6.3f} {n:>4d} {avg_k:>10.1f} {avg_loss:>12.2e}")
    ...
    for init_scale in INIT_SCALES:
        for noise in NOISES:
            muon_k = [r["K_epsilon"] for r in rows if r["algo"] == "Muon-Exact" and abs(r["init_scale"] - init_scale) < 1e-9 and abs(r["noise"] - noise) < 1e-9]
            sgd_k  = [r["K_epsilon"] for r in rows if r["algo"] == "SGD" and abs(r["init_scale"] - init_scale) < 1e-9 and abs(r["noise"] - noise) < 1e-9]
            if len(muon_k) == len(sgd_k) and len(muon_k) > 1:
                ...
                print(f"  init={init_scale:.4f} noise={noise:.3f}: t={t_stat:+.3f} p={p_val:.4f} {sign}  Cohen's d={d_effect:+.2f}")
