"""
E07 — Rank ratio scan: Muon vs SGD
==================================================
MS only. ALGOS=["Muon-Exact","SGD"]. D=50. RS=[2,5,10,25,50]. m = max(int(2*d*r), d*d). SEEDS=8.
"""
import os as _os
...
DIMS        = [50]
R           = 5  # default, overridden in loop
LR          = 0.01
RS          = [2, 5, 10, 25, 50]
NOISE       = 0.0
DIST        = "normal"
...
def run_ms(algo, d, r, lr, noise, dist, spectrum, kappa, init_scale, seed, iters):
    ...
    # ── Detailed logger (v2) ─────────────────────────
    logger = DetailedLogger(LOG_DIR, "E07_detailed", algo, {k: v for k, v in locals().items()
...
if __name__ == "__main__":
    total = len(ALGOS) * len(RS) * len(SEEDS)
    print(f"E07: {len(ALGOS)} algos x {len(RS)} ranks x {len(SEEDS)} seeds = {total} runs")
    print()
    rows = []
    idx = 0
    d = DIMS[0]
    for algo in ALGOS:
        for r in RS:
            for seed in SEEDS:
                idx += 1
                print(f"[E07 {idx}/{total}] {algo} r={r} seed={seed}", flush=True)
                row = run_ms(algo, d, r, LR, NOISE, DIST, SPECTRUM, KAPPA,
                             INIT_SCALE, seed, ITERS)
                rows.append(row)

    save_csv("E07_detailed", rows)

    print(f"\n{'='*60}")
    print("E07 统计汇总")
    print(f"{'='*60}")
    print(f"\n{'algo':<16s} {'r':>4s} {'r/d':>6s} {'n':>4s} {'avg K_eps':>10s} {'avg loss':>12s}")
    print("-" * 60)
    d = DIMS[0]
    for algo in ALGOS:
        for r in RS:
            subset = [r_ for r_ in rows if r_["algo"] == algo and r_["r"] == r]
            n = len(subset)
            if n == 0: continue
            avg_k = np.mean([r_["K_epsilon"] for r_ in subset])
            avg_loss = np.mean([r_["final_loss"] for r_ in subset])
            print(f"{algo:<16s} {r:>4d} {r/d:>6.2f} {n:>4d} {avg_k:>10.1f} {avg_loss:>12.2e}")

    print(f"\n{'='*60}")
    print("配对检验: Muon-Exact vs SGD (K_epsilon)")
    print(f"{'='*60}")
    for r in RS:
        muon_k = [r_["K_epsilon"] for r_ in rows if r_["algo"] == "Muon-Exact" and r_["r"] == r]
        sgd_k  = [r_["K_epsilon"] for r_ in rows if r_["algo"] == "SGD" and r_["r"] == r]
        if len(muon_k) == len(sgd_k) and len(muon_k) > 1:
            ...
            print(f"  r={r:>3d} (r/d={r/d:.2f}): t={t_stat:+.3f} p={p_val:.4f} {sign}  Cohen's d={d_effect:+.2f}")
    ...
    for r in RS:
        ...
        print(f"  r={r:>3d} (r/d={r/d:.2f}): SGD={sgd_flops:.2e}  Muon={muon_flops:.2e}  ratio={ratio:.2f}")
    print(f"\n{'='*60}")
    print("E07 完成")
    print(f"{'='*60}")
