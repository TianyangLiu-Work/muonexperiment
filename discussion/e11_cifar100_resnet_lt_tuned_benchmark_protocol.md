...
| SEL-6-state-distribution-logging | Every validation and final recipe run writes a trajectory occupancy trace with class exposure, tail quality, gradient-momentum cosine, and matched-head-gain local drift probes. | Using local Muon drift compatibility as a practical-performance explanation without state-distribution occupancy evidence. |
...
| TB-7-state-distribution-occupancy | Practical Muon mechanism wording can discuss why a selected recipe did or did not transfer to final performance. | Every selected validation and final run includes occupancy_trace.csv rows with class exposure, tail quality, gradient-momentum cosine, and matched-head-gain NS-vs-Fro local drift ratio. | local compatibility remains sampled-state evidence only |
