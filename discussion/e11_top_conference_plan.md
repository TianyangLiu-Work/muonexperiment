...
| Held-out checkpoint-transfer JVP benchmark | Across tail-rich 2000/5000/10000-step ResNet checkpoints, source-observed positive-control Spearman is `0.981 [0.9739, 0.988]` and early-layer prior Spearman is `0.9126 [0.9029, 0.9222]`, but scaled-JVP held-out layer-risk Spearman is `-0.3203 [-0.3562, -0.2845]` despite below-one threshold accuracy `1`. After source-fit early-layer residualization, observed residual Spearman is `0.9403 [0.9216, 0.959]`, while scaled-JVP residual Spearman is `-0.4872 [-0.5373, -0.4372]`. | predictive boundary |
...
| P0 | Predictive downstream-aware condition benchmark | A first held-out checkpoint-transfer benchmark is complete and now includes positive controls: source-observed drift and early-layer structure predict cross-checkpoint layer ordering, observed residuals remain predictive after source-fit depth adjustment, but scaled-JVP preserves only the below-one threshold direction and not raw or residual ranking. The next gate is a stronger downstream-aware condition score plus held-out architecture/dataset splits. | The theorem must look predictive beyond the synthetic construction and beyond one ResNet checkpoint family. |
...
optimizer performance. The checkpoint-transfer benchmark shows that
source-observed drift and early-layer structure transfer the layer-risk ranking
across tail-rich checkpoints, while the current scaled-JVP readout transfers
the below-one direction but not that ranking. The residual version shows
observed source residuals still transfer after source-fit early-layer
adjustment, while scaled-JVP residuals remain inverted. This narrows the
failure to the condition score rather than target-checkpoint noise or a pure
layer-depth effect. The standard reporting
