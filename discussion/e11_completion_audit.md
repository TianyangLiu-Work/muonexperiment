...
|---|---|---|
| Larger-dataset long-tail evidence remains incomplete. | The paper now includes CIFAR-100-LT ResNet18 local diagnostics, tail-rich controls, standard reporting, recipe pilots, and a negative NS-Muon final-training pilot, but still frames these as mechanism and benchmark-context evidence rather than a broad long-tail benchmark result. | Add ImageNet-LT or iNaturalist-style diagnostics under a preregistered protocol. |
| Practical Muon is not fully explained as an optimizer. | The paper only claims selected-state/local compatibility and explicitly separates this from optimizer-level performance; the CIFAR-100-LT NS-Muon final-training pilot is negative. | Complete tuned practical Muon/AdamW/SGD/class-balanced validation selection and untouched final paired seeds, while sampling matched-gain trajectory diagnostics. |
...
| The rank boundary is not yet a natural-task predictor. | The synthetic boundary is presented as a controlled mechanism check; the condition-score program is frozen but still registered-not-ready until unspent v5 ResNeXt50-32x4d and CIFAR-10 cross-partition final splits finish and pass. | Run the frozen v5 final evaluator on completed final layer tables and preserve negative outcomes under the interpretation plan. |
