...
| Architecture generality | All-layer ResNet18 JVP diagnostics are present, but v5 ResNeXt50-32x4d and CIFAR-10 cross-partition final condition-score outputs are still pending. | Consume the submitted v5 final Slurm outputs with the frozen evaluator and preserve pass/fail outcomes without refit. |
| Boundary predictiveness | The synthetic boundary is controlled and the current condition-score program remains registered-not-ready until both v5 final splits pass. | Use only the frozen v5 score on unspent final splits, then update the claim ladder under the pre-output interpretation plan. |
