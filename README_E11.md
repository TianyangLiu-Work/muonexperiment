...
make e11-cifar-resnet-lt-tuned-benchmark-final-robustness-plan # pre-register nonparametric/bootstrap sensitivity and dropped-seed policies
...
- `scripts/e11_write_condition_score_v5_final_power_audit.py`: completed-final v5 detectable-effect audit that preserves the failed residual gates as a negative-transport boundary rather than a score-repair opportunity.
- `scripts/e11_write_condition_score_v5_final_interpretation_plan.py`: completed-final v5 interpretation lock that maps observed pass/fail patterns to allowed claims, boundary interpretations, and forbidden post-hoc retuning actions.
...
- `scripts/e11_write_pdf_render_boundary_audit.py`: rendered-PDF inspection boundary audit that checks PDF bytes, rendered text-layer claim anchors, and page metadata through the pypdf fallback while keeping preferred LaTeX clean-checkout reproducibility separate.
...
- `scripts/e11_write_cifar100_resnet_lt_tuned_benchmark_final_robustness_plan.py`: pre-final robustness plan for exact sign-flip, bootstrap, sign-count sensitivity, dropped-seed policy, and claim downgrades.
