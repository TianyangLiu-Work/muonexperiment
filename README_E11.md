...
mean-Spearman MDE before final rows exist, so a failed or null final split is
interpreted as underpowered, small-effect, or negative transport evidence only
under the audited detectable-effect scale.
...
the outcome-to-claim state machine before the final layer tables exist, including
